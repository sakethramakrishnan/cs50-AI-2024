...
def transition_model(corpus, page, damping_factor):
    ...
    if len(corpus[page]) < 1:
        for page in corpus.keys():
            probs[page] = 1 / num_pages  # randomly initializing to all the same prob

    else:
        rand_factor = (1 - damping_factor) / num_pages
        even_factor = damping_factor / len_page

        for page_0 in corpus.keys():
            if page_0 not in corpus[page]:
                ...
                probs[page_0] = rand_factor
            else:
                probs[page_0] = even_factor + rand_factor
...
def iterate_pagerank(corpus, damping_factor):
    ...
    post = {}
    for page in corpus:
        prior[page] = 1 / num_pages  # set everything to same probability
        ...
        for page in corpus:
            ...
            for page_link in corpus:
                ...
                if page in corpus[page_link]:
                    tmp += (prior[page_link] / len(corpus[page_link]))

                # "A page that has no links at all should be interpreted as having one link for every page in the corpus (including itself)."
                if len(corpus[page_link]) == 0:
                    tmp += (prior[page_link]) / len(corpus)
            tmp *= damping_factor
            tmp += (1 - damping_factor) / num_pages

            post[page] = tmp

        diff = max([abs(post[x] - prior[x]) for x in prior])
        if diff < 0.001:
            break
        else:
            prior = post.copy()
